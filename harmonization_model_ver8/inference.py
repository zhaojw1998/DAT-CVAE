...
try:
    weight_path = '/gpfsnyu/scratch/jz4807/model-weights/derek/harmonization_model_ver8/20210926-00-19-19_variation-for-context-dependency/models/ad-ptvae_param_099.pt'
...
try:
    dataset = np.load('../data.npy', allow_pickle=True).T
except:
    dataset = np.load('/gpfsnyu/scratch/jz4807/data.npy', allow_pickle=True).T
np.random.seed(0)
...
print(len(val_set))
WRITE_PATH = './harmonization_model_ver8/write'
if not os.path.exists(WRITE_PATH):
    os.makedirs(WRITE_PATH)


chord_1, melody_1, _ = val_set.__getitem__(8)
...
chord_2, melody_2, _ = val_set.__getitem__(2749)
...
chord_3, melody_3, _ = val_set.__getitem__(3413)
...
chord_4, melody_4, _ = val_set.__getitem__(5126)
...
midi = pyd.PrettyMIDI('./harmonization_model_ver8/modal change/recon_1.mid')
...
midi = pyd.PrettyMIDI('./harmonization_model_ver8/modal change/recon_2.mid')
melody = melody_data2matrix(midi.instruments[0], midi.get_downbeats())
melody = val_set.truncate_melody(melody)
melody_2_modal_change = torch.from_numpy(melody).float().cuda().unsqueeze(0)

midi = pyd.PrettyMIDI('./harmonization_model_ver8/modal change/recon_3.mid')
melody = melody_data2matrix(midi.instruments[0], midi.get_downbeats())
melody = val_set.truncate_melody(melody)
melody_3_modal_change = torch.from_numpy(melody).float().cuda().unsqueeze(0)

midi = pyd.PrettyMIDI('./harmonization_model_ver8/modal change/recon_4.mid')
melody = melody_data2matrix(midi.instruments[0], midi.get_downbeats())
melody = val_set.truncate_melody(melody)
