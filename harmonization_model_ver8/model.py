...
class VAE(nn.Module):
    ...
    def __init__(self, max_simu_note=4+2, max_pitch=11, min_pitch=0,
        ...
        discr_TEL = nn.TransformerEncoderLayer(note_emb_size*2, discr_nhead, discr_hid_size, discr_dropout, batch_first=True)
        self.discr_TE = nn.TransformerEncoder(discr_TEL, discr_nlayer)
        self.discr_out_linear = nn.Linear(note_emb_size*2, 12)   #only keep melody pitch dimension (0~127) 

        self.discr_bar_pos_embedding = nn.Linear(self.num_step//4, self.note_emb_size*2)
        self.discr_beat_pos_embedding = nn.Linear(4, self.note_emb_size*2)
        self.discr_quaver_pos_embedding = nn.Linear(4, self.note_emb_size*2)
    ...
    def encoder(self, chord, lengths, melody):
        ...
        #plus_melody = melody.unsqueeze(2).repeat(1, 1, self.max_simu_note, 1)
        #chord_plus_melody = torch.cat([chord, plus_melody], dim=-1)
        #chord_plus_melody = chord_plus_melody.view(-1, self.max_simu_note, 2*self.note_emb_size)
        #chord_plus_melody = pack_padded_sequence(chord_plus_melody, lengths.view(-1), batch_first=True, enforce_sorted=False)
    ...
    def positional_embedding(self, inputs, pos_type):
        #inputs: (B, num_step, note_embed_size) or (B, num_step*4, note_embed_size)
        if pos_type == 'bar':
            pos_embedding = torch.zeros((inputs.shape[0], inputs.shape[1], 8)).to(self.device)
            for i in range(0, inputs.shape[1], inputs.shape[1]//8):
                pos_embedding[:, i: i+inputs.shape[1]//8, i//(inputs.shape[1]//8)] = 1.
            pos_embedding = self.discr_bar_pos_embedding(pos_embedding)
        elif pos_type == 'beat':
            pos_embedding = torch.zeros((inputs.shape[0], inputs.shape[1], 4)).to(self.device)
            for i in range(0, inputs.shape[1], inputs.shape[1]//8//4):
                pos_embedding[:, i: i+inputs.shape[1]//8//4, (i//(inputs.shape[1]//8//4))%4] = 1.
            pos_embedding = self.discr_beat_pos_embedding(pos_embedding)
        elif pos_type == 'quaver':  #only for time quantized in 16th
            pos_embedding = torch.zeros((inputs.shape[0], inputs.shape[1], 4)).to(self.device)
            for i in range(0, inputs.shape[1]):
                pos_embedding[:, i, i%4] = 1.
            pos_embedding = self.discr_quaver_pos_embedding(pos_embedding)
        return inputs + pos_embedding

    def discr(self, mu, masked_melody, masked_idx):
        #mu: (B, note_embed_size)
        #masked_melody: (B, num_step*4, note_embed_size)
        #masked_idx: [[B, B, B, B, ...], [T, T, T, T, ...]]
        masked_melody = torch.cat([masked_melody, mu.unsqueeze(1).repeat(1, masked_melody.shape[1], 1)], axis=-1)
        masked_melody = self.positional_embedding(masked_melody, 'bar')
        masked_melody = self.positional_embedding(masked_melody, 'beat')
        masked_melody = self.positional_embedding(masked_melody, 'quaver')
        output = self.discr_TE(masked_melody)
        output = self.discr_out_linear(output)  #(B, num_step*4, 128)

        return output[masked_idx[0], masked_idx[1], :]
    ...
    def forward(self, chord, melody, masked_melody, inference=False, sample=True,
        ...
        # chord: (B, num_step, max_simu_note, note_size), note_size = MIDI(0~127) + sos(128) + eos(129) + pad(130) + Vacant(131~133)
        # melody: (B, num_step*4, 28), 28 = chroma(0~11) + Vacant(12~14) + hold(15) + rest(16) + mask(17) + register(10)
        # masked_melody: (B, num_step*4, 134)

        lengths = self.get_len_index_tensor(chord)  # lengths: (B, num_step)
        chord = self.index_tensor_to_multihot_tensor(chord)
        chord = self.enc_note_embedding(chord)    #(B, num_step, max_simu_note, note_emb_size)
        
        melody = self.enc_note_embedding(melody)    #(B, num_step*4, note_emb_size)
        ...
        masked_melody = self.enc_note_embedding(masked_melody)    #(B, num_step*4, note_emb_size)
...
if __name__ == '__main__':
    ...
    for i, (chord, melody, masked_melody) in enumerate(data_loader):
        chord = chord.long().cuda() # this is an indice tensor
        melody = melody.float().cuda()  # this is a one-hot tensor
        masked_melody = masked_melody.float().cuda()
        print(chord.shape, melody.shape, masked_melody.shape)
        pitch_outs, dist, mask_output, mask_idx = model(chord, melody, masked_melody, inference=True, sample=None, teacher_forcing_ratio1=0, teacher_forcing_ratio2=0)
        mask_gt = melody[mask_idx[0], mask_idx[1], :12]
        print(pitch_outs.shape, mask_output.shape, mask_gt.shape)
        print(mask_output.max(-1)[-1])
        print(mask_gt.max(-1)[-1])
