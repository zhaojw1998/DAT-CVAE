...
class Nottingham(Dataset):
    def __init__(self, dataset, length, step_size, chord_fomat='chroma', shift_high=6, shift_low=-5, mask_ratio=1.):
    ...
    def __getitem__(self, idx):
        ...
        #130D to 28D
        melody_28D = self.truncate_melody(melody) 
        #mask by transposition
        mask_shift = random.randint(-5, 6)
        masked_melody = np.concatenate((np.roll(melody[:, :128], mask_shift, -1), melody[:, -2:]), axis=-1)
        masked_melody = self.truncate_melody(masked_melody)    #128, 28
        #masked_melody = self.mask_melody(melody)
        pitch_mask = 17
        onsets, pitch_idx = np.nonzero(masked_melody[:, :12])
        masked_melody[onsets, pitch_mask] = 1.
        ...
        chord = target_to_3dtarget(pr_mat = chord,
                                    ...
                                    pitch_sos_ind=12,
                                    pitch_eos_ind=13)
        return chord, melody, melody_28D, masked_melody
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(chord.shape, melody.shape, masked_melody.shape)
        print(chord)
        print(np.nonzero(melody))
        print(np.nonzero(masked_melody))
        break